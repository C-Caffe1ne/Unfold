...
description: "Orca에서 Codex 멀티에이전트로 Unfold 개발을 계획·분배·감독한다. 개발 계획, 구현, QA, BM 우선순위, 다음 작업, 부분 재실행, 업데이트, 수정, 보완, 이전 결과 개선 요청에는 반드시 이 스킬을 사용한다."
---
...
| 워커 | 역할 파일 | 스킬 | 초기 출력 |
|---|---|---|---|
| product-strategist | `.Codex/agents/product-strategist.md` | `unfold-product-planning` | `.Codex/skills/unfold-orchestrator/_workspace/01_product_strategy.md` |
| avalonia-architect | `.Codex/agents/avalonia-architect.md` | `unfold-runtime-audit` | `.Codex/skills/unfold-orchestrator/_workspace/02_architecture.md` |
| release-qa | `.Codex/agents/release-qa.md` | `unfold-release-qa` | `.Codex/skills/unfold-orchestrator/_workspace/03_release_qa.md` |
| implementation-engineer | `.Codex/agents/implementation-engineer.md` | `unfold-implementation` | Task별 구현 보고서 |
| ui-visual-auditor | `.Codex/agents/ui-visual-auditor.md` | `unfold-ui-visual-audit` | UI 시각 감사 보고서 |
| ux-flow-auditor | `.Codex/agents/ux-flow-auditor.md` | `unfold-ux-flow-audit` | 사용자 흐름 감사 보고서 |
| ui-accessibility-qa | `.Codex/agents/ui-accessibility-qa.md` | `unfold-ui-accessibility-qa` | 접근성·반응형 QA 보고서 |

## Phase 0: 컨텍스트 확인

1. 현재 브랜치와 Git 상태를 확인하고 사용자의 기존 변경을 보존한다.
2. `.Codex/skills/unfold-orchestrator/_workspace/`가 없으면 초기 실행한다.
...
2. `.Codex/skills/unfold-orchestrator/_workspace/00_input.md`에 목표, 브랜치, 기준 문서, 금지 범위, 현재 검증 상태를 기록한다.
...
3. Codex 워커는 `orca orchestration worker-start --worktree current --agent Codex --model opus`로 실행한다.
