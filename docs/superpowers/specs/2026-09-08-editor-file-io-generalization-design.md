...
따라서 **총량 상한 64MB**를 함께 둔다.

이 값은 오늘 합법인 문서를 하나도 배제하지 않는다. 현재 최대치가 24MB이기
때문이다. 512×512를 쓰면 프레임·레이어 수가 자연히 제한된다(24프레임 2레이어,
또는 8프레임 8레이어).
...
연속하는 것인데, 문서 총량이 64MB로 묶여 있으므로 상한이 존재한다. 실사용
...
- **`editorMaxSheetDataURLBytes`** 8MB — 12,288×512 PNG가 넘을 수 있다
- **`characterDisplaySize`** 192 — 512px 스프라이트의 데스크톱펫 표시
- **`PixelDocumentCodec.maximumSourceBytes`** 48MB — 총량 상한 64MB와의 관계
