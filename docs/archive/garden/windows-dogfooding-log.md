> **보관 문서 · Garden 실험 전 Cat 검증 양식 / 97f9a84**
> 실행되지 않은 당시 양식입니다. 원문의 Garden 착수 조건과 유예 목록은 현재 지침이 아닙니다.
> 현재 Windows 검증은 [현재 체크리스트](../../windows-dogfooding-log.md)를 사용합니다.

# Unfold Windows Dogfooding Log

Windows-only verification of the Cat MVP. The macOS record lives in
[당시 macOS 기록](../../validation/2026-09-11-macos-dogfooding.md) and is a separate document on purpose — a
