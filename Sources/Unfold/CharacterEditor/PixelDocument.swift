...
struct PixelDocument: Equatable {
    ...
    var playbackMode: PixelPlaybackMode = .loop
    var playbackStart = 0
    /// nil means through the current last frame.
    var playbackEnd: Int? = nil
    var palette: [UInt32] = PixelDocument.defaultPalette
    ...
    mutating func draw(tool: PixelTool, from start: PixelPoint, to end: PixelPoint,
        ...
        guard layers.indices.contains(layer), !layers[layer].isLocked, (0..<frameCount).contains(frame) else { return }
        if tool == .fill {
    ...
    mutating func insertFrame(after index: Int, duplicate: Bool) {
        ...
        if !frameSettings.isEmpty {
            ...
            frameSettings.insert(duplicate ? settings(at: index) : PixelFrameSettings(), at: index + 1)
        }
        if playbackStart > index { playbackStart += 1 }
        if let end = playbackEnd, end > index { playbackEnd = end + 1 }
        for l in layers.indices {
    ...
    mutating func removeFrame(at index: Int) {
        ...
        if !frameSettings.isEmpty { normalizeFrameSettings(); frameSettings.remove(at: index) }
    ...
    mutating func moveFrame(from: Int, to: Int) {
        ...
        if !frameSettings.isEmpty {
            normalizeFrameSettings()
            frameSettings.insert(frameSettings.remove(at: from), at: to)
        }
        for l in layers.indices {
