...
{
    public int TotalSeconds => Entries.Sum(item => item.Seconds);
    public int DaysWithBreaks => Days.Count(day => day.Count > 0);
    public byte[] Csv()
    {
        var csv = new StringBuilder("confirmed_at,routine_id,routine_name,planned_seconds,character_id,profile_id,profile_name\r\n");
